...
| `assets/images/*.webp` | 원래 각 페이지 HTML에 base64로 인라인되어 있던 사진들 (파일로 분리 + 압축, 내용 해시로 중복 제거) |
| `og-image.webp` | 소셜 미리보기용 이미지 (원본 `/og-image.jpg`를 압축) |
...
- **인라인 이미지는 파일로 분리 + 압축했습니다.** 원본 사이트는 사진을 `data:image/...;base64,...` 형태로 HTML 안에 직접 박아 넣는 구조라서, 홈 페이지 하나가 1.5MB에 달했습니다. `mirror.js`가 이런 base64 이미지를 감지해 (필요 시 가로 1600px로 축소 후) WebP로 재압축하고, `assets/images/`에 실제 파일(내용 해시로 중복 제거)로 저장한 뒤 HTML에서는 그 파일 경로만 참조하도록 바꿨습니다. 화면에 보이는 결과물은 원본과 사실상 동일하고, 이미지 총 용량은 약 40~50% 줄었으며, HTML은 페이지당 10~23KB 수준으로 줄어 사람이 읽고 diff로 비교하기 쉬워졌습니다. **앞으로 이 저장소에 외부 이미지를 추가로 가져와 붙일 때도 원본 그대로 넣지 않고 이 방식(리사이즈 + WebP 재압축)을 기본으로 적용합니다.**
